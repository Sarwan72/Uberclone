...
const Home = () => {
  ...
  const handlePickupChange = async (e) => {
    ...
    try {
      ...
      setPickupSuggestions(
        response.data.features.map((f) => f.properties.formatted)
      );
    } catch (err) {
  ...
  //   // TODO: Replace with your backend for fare calculation
  //   setFare({
  //     car: 120,
  //     auto: 80,
  //     bike: 50,
  ...
  async function findTrip() {
    ...
    try {
      // Make sure you already have pickup & destination with {lat, lon, address}
      const response = await axios.post("http://localhost:4000/api/ride/fare", {
        pickup: selectedPickup, // example: { lat: 28.6139, lon: 77.2090, address: "Delhi" }
        destination: selectedDest, // example: { lat: 28.4595, lon: 77.0266, address: "Gurgaon" }
      });

      setFare(response.data); // update state with {car, auto, moto}
    } catch (err) {
    ...
    <div className="relative  h-screen overflow-hidden">
      ...
      >
        {fare ? (
          ...
          <p className="text-center text-gray-500">Loading fare...</p>
        )}
      </div>
